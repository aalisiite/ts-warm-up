...
function convertToUSD(price: number) {
    return roundPrice(price, 1.4);
}
// You are allowed to change this function
function convertToBRL(price: number) {
    return addTransactionFee(price *= 6.31);
}

function addTransactionFee(price: number) {
    const fee = roundPrice(price, 0.1)
    return Math.round((price + fee) * 100) / 100;
}

function roundPrice(price: number, rate: number) {
